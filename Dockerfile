...
# 1) deps only (cached layer). No uv.lock + no --frozen on purpose: the lock pins
#    pythonhosted download URLs which ignore the mirror; resolving fresh against
#    UV_INDEX_URL (above) downloads from the fast mirror instead.
COPY pyproject.toml ./
RUN uv sync --extra server --no-dev --no-install-project
